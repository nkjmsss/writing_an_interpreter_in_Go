...
import (
	...
	"github.com/nkjmsss/writing_an_interpreter_in_Go/ast"
...
func TestLetStatements(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if ss := len(program.Statements); ss != 1 {
			t.Fatalf(
...
func TestReturnStatements(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if s := returnStmt.TokenLiteral(); s != "return" {
			t.Fatalf(
...
func testBooleanLiteral(t *testing.T, exp ast.Expression, value bool) bool {
	...
	if !ok {
		t.Errorf(
			"bo not *ast.BooleanLiteral. got=%T",
			bo,
		)
		return false
	}
...
func testInfixExpression(
	...
	if !ok {
		t.Errorf(
			"exp is not ast.InfixExpression. got=%T(%s)",
			exp,
			exp,
		)
		return false
